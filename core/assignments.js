...
const ASSIGNMENTS = [
  ...
  // === ASSIGNMENT 5 — EXP5 · Viajes / Lugares ===
  {
    ...
    expLabel: 'exp5',
    instructions: 'Cada frase usa SOLO palabras de Experience 5 (Viajes / Lugares). Filtra el catálogo con la pestaña ✈️ EXP5. 🗣️ DI CADA FRASE EN VOZ ALTA.',
    type: 'sentence-building',
    items: [
      { es: 'China',                expected: 'zhōngguó' },
      { es: 'Beijing',              expected: 'běijīng' },
      { es: 'En Beijing',           expected: 'zài běijīng' },
      { es: 'En China',             expected: 'zài zhōngguó' },
      { es: 'El restaurante',       expected: 'fànguǎn' },
      { es: 'En el restaurante',    expected: 'zài fànguǎn' },
      { es: 'En el hotel',          expected: 'zài fàndiàn' },
      { es: 'Tienda',               expected: 'shāngdiàn' },
      { es: 'En la tienda',         expected: 'zài shāngdiàn' },
      { es: 'Ir a China',           expected: 'qù zhōngguó' },
    ],
    pointsPerItem: 10,
    parentInsight: {
      title: 'Tu hijo/a sabe ubicar lugares y hablar de viajes en chino',
      bullets: [
        'Países y ciudades: "zhōngguó" (China), "běijīng" (Beijing)',
        'Localización con "zài": "zài běijīng" (en Beijing), "zài zhōngguó" (en China)',
        'Lugares cotidianos: "fànguǎn" (restaurante), "fàndiàn" (hotel), "shāngdiàn" (tienda)',
        'Acciones de viaje: "qù zhōngguó" (ir a China)',
      ],
      encouragement: 'En el próximo viaje, pídele que diga "zài fàndiàn" o "zài fànguǎn" cuando lleguen — verás que ya ubica los lugares en chino.',
    },
  },
  // === ASSIGNMENT 6 — EXP6 · Casa / Actividades ===
  {
    ...
    expLabel: 'exp6',
    instructions: 'Cada frase usa SOLO palabras de Experience 6 (Casa / Actividades). Filtra el catálogo con la pestaña 🏠 EXP6. 🗣️ DI CADA FRASE EN VOZ ALTA.',
    type: 'sentence-building',
    items: [
      { es: 'Trabajar',                expected: 'gōngzuò' },
      { es: 'Trabajo (cosa)',          expected: 'gōngzuò' },
      { es: 'Mirar',                   expected: 'kàn' },
      { es: 'Mirar películas',         expected: 'kàn diànyǐng' },
      { es: 'Hacer / Cocinar',         expected: 'zuò' },
      { es: 'Cocinar comida',          expected: 'zuò cài' },
      { es: 'Comprar cosas',           expected: 'mǎi dōngxi' },
      { es: 'Llamar por teléfono',     expected: 'dǎ diànhuà' },
      { es: 'Cosa',                    expected: 'dōngxi' },
      { es: 'Película',                expected: 'diànyǐng' },
    ],
    pointsPerItem: 10,
    parentInsight: {
      title: 'Tu hijo/a habla de actividades cotidianas en chino',
      bullets: [
        'Trabajar: "gōngzuò" (trabajo / trabajar)',
        'Ver películas: "kàn diànyǐng"',
        'Cocinar: "zuò cài" (hacer comida)',
        'Compras: "mǎi dōngxi" (comprar cosas)',
        'Llamadas: "dǎ diànhuà" (hacer una llamada)',
      ],
      encouragement: 'Cuando vean una película juntos pregúntale "¿qué estamos haciendo?" — debe responder "wǒmen kàn diànyǐng".',
    },
  },
  // === ASSIGNMENT 7 — EXP7 · Personas / Preguntas ===
  {
    ...
    expLabel: 'exp7',
    instructions: 'Cada frase usa SOLO palabras de Experience 7 (Personas / ¿?). Filtra el catálogo con la pestaña 🙋 EXP7. 🗣️ DI CADA FRASE EN VOZ ALTA.',
    type: 'sentence-building',
    items: [
      { es: '¿Quién?',              expected: 'shéi' },
      { es: '¿Qué?',                expected: 'shénme' },
      { es: '¿Cuál?',               expected: 'nǎ' },
      { es: '¿Dónde?',              expected: 'nǎr' },
      { es: '¿Por qué?',            expected: 'wèishénme' },
      { es: '¿Cuántos? (≤10)',      expected: 'jǐ' },
      { es: '¿Cuánto / Cuántos?',   expected: 'duōshǎo' },
      { es: '¿Cómo?',               expected: 'zěnme' },
      { es: 'Persona',              expected: 'rén' },
      { es: '¿Qué persona?',        expected: 'shénme rén' },
    ],
    pointsPerItem: 10,
    parentInsight: {
      title: 'Tu hijo/a puede hacer preguntas en chino',
      bullets: [
        'Pronombres interrogativos: "shéi" (quién), "shénme" (qué), "nǎ" (cuál)',
        'Lugar y razón: "nǎr" (dónde), "wèishénme" (por qué)',
        'Cantidad: "jǐ" (cuántos ≤10), "duōshǎo" (cuántos en general)',
        'Modo: "zěnme" (cómo)',
        'Sustantivo base: "rén" (persona)',
      ],
      encouragement: 'Jueguen "20 preguntas" pero en chino. Cada respuesta que reciba en chino vale doble cariño. 💛',
    },
  },
  // === ASSIGNMENT 8 — EXP8 · Números / Partículas ===
  {
    ...
    expLabel: 'exp8',
    instructions: 'Cada frase usa SOLO palabras de Experience 8 (Números / Partículas). Filtra el catálogo con la pestaña 🔢 EXP8. 🗣️ DI CADA FRASE EN VOZ ALTA.',
    type: 'sentence-building',
    items: [
      { es: 'Uno',                  expected: 'yī' },
      { es: 'Dos',                  expected: 'èr' },
      ...
      { es: 'Mamá (con 妈)',         expected: 'māma' },
      { es: '¿Sí o no? (partícula)', expected: 'ma' },
      { es: 'Estoy bien (con 了)',  expected: 'hǎo le' },
      { es: 'No es bueno (con 不)', expected: 'bù hǎo' },
      { es: 'No (frente a verbo)',  expected: 'bù' },
    ],
    pointsPerItem: 10,
    parentInsight: {
      title: 'Tu hijo/a domina números y partículas claves',
      bullets: [
        'Números 1–10: "yī, èr, sān, sì, wǔ, liù, qī, bā, jiǔ, shí"',
        'Partícula de pregunta: "ma" — convierte una frase en pregunta',
        'Partícula de cambio: "le" — algo ya pasó o cambió',
        'Negación: "bù" antes del verbo, "bù hǎo" (no bueno)',
      ],
